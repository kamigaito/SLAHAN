...
You can also use HDF5 format as an output file, if you set a flag `--use_hdf5_output` as True.
In this HDF5 format, the structure of the extracted features are similar to the structure of [AllenNLP ELMo features](https://github.com/allenai/allennlp/blob/master/tutorials/how_to/elmo.md).
You can read this hdf5 formatted features with a `h5py` module.
The following is an example of reading hdf5 formatted features.
```
python
> import h5py
> h5py_file = h5py.File("bert_features.hdf5", 'r')
> features = h5py_file.get("0") # decide an extraction target sentence.
> print(len(features))
4
> layer = features[0] # decide an extraction target layer
> print(len(layer)) # print a sentence length
35
> token_id = 3 # decide an extraction target token.
> vector = layer[token_id] # extract a target vector
> tokens = h5py_file.get("txt-0") # extract target tokens
> print(tokens[token_id]) # print a corresponding token
kind
```

If you set a flag `--use_hdf5_output` as True, you can additionally use the following options:
- `--no_truncation` If this value is True, the truncation of input sequences will become an error.
- `--ignore_separator_token` If this value is True, vectors of the separator token [SEP] will be removed from the output file. This option is only supported on the HDF5 file format.
- `--ignore_class_token` If this value is True, vectors of the class token [CLS] will be removed from the output file. This option is only supported on the HDF5 file format.
- `--word_embeddings` If True, sub-word-based embeddings are merged as word-based embeddings on the output file. This option is only supported on the HDF5 file format.
- `--word_composition` This value decides the merging operation for creating word embeddings. ''avg'' uses averaging of sub-word embeddings, ''sum'' uses summation of sub-word embeddings, ''left-most'' uses embeddings of left-most sub-words in words as word representations, ''right-most'' uses embeddings of right-most sub-words in words as word representations. The default value  is set to ''avg''. This option is only supported on the HDF5 file format.

### Convert JSON to HDF5 file format

You can convert a JSON formatted feature file to a HDF5 formatted one by using a script `json2hdf5.py`.
```
python jsonl2hdf5.py \
  --input_file=/path/to/input.txt \
  --jsonl=/path/to/bert_features.jsonl \
  --hdf5=/path/to/bert_features.hdf5 \
  --vocab_file=/path/to/vocab.txt \
```
In `jsonl2hdf5.py`, you can additionally use the following options:
- `--ignore_separator_token` If this value is True, vectors of the separator token [SEP] will be removed from the output file.
- `--ignore_class_token` If this value is True, vectors of the class token [CLS] will be removed from the output file.
- `--word_embeddings` If True, sub-word-based embeddings are merged as word-based embeddings on the output file.
