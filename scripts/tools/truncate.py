...
for line_ref in ref_file:
    ...
    for i in range(0, len(toks_ref)):
        if toks_ref[i] == "1":
            comp_lst_ref.append(toks_inp[i])
        if toks_sys[i] == "1":
            comp_lst_sys.append(toks_inp[i])
    comp_ref = " ".join(comp_lst_ref)
    comp_sys = " ".join(comp_lst_sys)
    # reference length limit
    idx = len(comp_lst_sys)
    while len(comp_ref.encode("utf-8")) < len(comp_sys.encode("utf-8")):
        idx -= 1
        comp_sys = " ".join(comp_lst_sys[:idx])
